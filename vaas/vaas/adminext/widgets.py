    ...
    def __init__(
        ...
        **kwargs,
        ...
        widgets: tuple[forms.Select, forms.Select, forms.TextInput] = (
            forms.Select(choices=variables),
            forms.Select(choices=operators),
            forms.TextInput(),
        )
...
class ComplexConditionWidget(forms.MultiWidget):
    def __init__(
        ...
        **kwargs,
        ...
        self.base_widget = ConditionWidget(
            self.variables, self.operators, *args, **kwargs
        )
        widgets = [
            self.base_widget,
        ]
    ...
    def decompress(self, value: str) -> list[str]:
        ...
        if len(result) > 1:
            self.widgets = [
                ConditionWidget(self.variables, self.operators)
    ...
    def value_from_datadict(self, data, files, name):
        parts = [
            self.base_widget.value_from_datadict(data, files, name + "_%s" % i)
            for i in self.get_ids(data, name)
        ]
...
class ComplexRedirectConditionField(forms.MultiValueField):
    def __init__(self, **kwargs):
        domains: tuple[tuple[str, str], ...] = tuple(
            (domain.pk, domain.domain) for domain in DomainMapping.objects.all()
        )
        fields = (
...
class RewriteGroupsInput(forms.TextInput):
    def get_context(self, name, value, attrs):
        ...
        context["widget"]["attrs"].update({"disabled": value is None})
        return context
...
class RewriteGroupsWidget(forms.MultiWidget):
    ...
    def __init__(self):
        widgets = (
            ...
            RewriteGroupsInput(
                attrs={
                    "placeholder": "Place here your regular expression for rewrite source groups"
                }
            ),
        )
