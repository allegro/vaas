...
class DirectorAdmin(HistoryMixinAdmin, SimpleHistoryAdmin, AuditableModelAdmin):
    ...
    def get_clusters(self, obj: Director) -> str:
    ...
    def _custom_flag(self, flag):
        if flag:
            return format_html(
                ...
                "<a class='btn btn-xs btn-success' href='#'><i class='fa-solid fa-circle-check'></i></a>"
                "</div>"
            )
        else:
            return format_html(
...
class BackendAdmin(HistoryMixinAdmin, SimpleHistoryAdmin, AuditableModelAdmin):
    ...
    def get_queryset(self, request):
        return (
            super(BackendAdmin, self)
            .get_queryset(request)
            .prefetch_related("tags", "director")
            .extra(
                select={
                    "status": "SELECT status from monitor_backendstatus"
        ...
        if obj.status:
            if obj.status == "Healthy":
                return format_html(
                    ...
                    "<i class='fa-solid fa-circle-check'></i></a></div>"
                )
            else:
                return format_html(
            ...
            return format_html(
                "<div class='span13 text-center'><a class='btn btn-xs' href='#'>"
                "<i class='fa-solid fa-power-off'></i></a></div>"
            )
