...
class VarnishPurgerTest(TestCase):
    ...
    def test_should_purge_active_varnishes(self, request_mock):
        expected_message_varnish1 = [
            "varnish http response code: 200, url=http://example.com/modified_resource/, headers=[('Host', 'example.com'), ('header1', 'value1')], server=127.0.0.1:80",
            "varnish http response code: 200, url=http://example.com/modified_resource/, headers=[('Host', 'example.com'), ('header1', 'value2')], server=127.0.0.1:80"
        ]
        expected_message_varnish2 = [
            "varnish http response code: 200, url=http://example.com/modified_resource/, headers=[('Host', 'example.com'), ('header1', 'value1')], server=127.0.0.2:80",
            "varnish http response code: 200, url=http://example.com/modified_resource/, headers=[('Host', 'example.com'), ('header1', 'value2')], server=127.0.0.2:80"
        ]
        servers = [VarnishServer(ip='127.0.0.1'), VarnishServer(ip='127.0.0.2')]
        result = VarnishPurger().purge_url('http://example.com/modified_resource/', servers, headers={'header1': ['value1', 'value2']})
